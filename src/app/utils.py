...
def make_scatter(data, cluster=False, size=False):
    ...
    # passando como kwargs o dicionario config para o scatterplot
    ...
    plt.hlines(y=txCorRacaPreta, 
               ...
               colors='black', alpha=0.6, linestyles='--', 
               label=f"Pessoas Pretas Geral: {100*txCorRacaPreta:.0f}%")

    plt.vlines(x=txGenFeminino, 
               ymin=data["txCorRacaPreta"].min(),
               ymax=data["txCorRacaPreta"].max(), 
               colors='tomato', alpha=0.6, linestyles='--', 
               label=f"Mulheres Geral: {100*txGenFeminino:.0f}%")
